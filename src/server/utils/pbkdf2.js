const buffer0 = new Int32Array(16);
const buffer512 = new Int32Array(16);
buffer512[0] = 512;

const bitLength = 512;

const C = (() => {
    let h = new Int32Array(b64decode('' +
...
const Ax = new Int32Array(b64decode(
...
const XLPS = (x, y) => {
    ...
    xor512(rXLPS, y);
    for (let i = 0; i < 8; i++) {
        let z0, z1, k = get8(rXLPS, i) << 1;
        z0 = Ax[k];
        z1 = Ax[k + 1];
        for (let j = 1; j < 8; j++) {
            k = (j << 9) + (get8(rXLPS, (j << 3) + i) << 1);
            z0 = z0 ^ Ax[k];
            z1 = z1 ^ Ax[k + 1];
        }
        x[i << 1] = z0;
...
const stribog = (data) => {
    ...
    let d = new Uint8Array(buffer(data));
    let n = d.length;
    let r = n % 64, q = (n - r) / 64;
    for (let i = 0; i < q; i++) {
        let m = get512(new Uint8Array(d.buffer, i * 64, 64));
        g(h, N, m);
        add512(N, buffer512);
        add512(sigma, m);
    }

    let dStage3 = new Uint8Array(d.buffer, q * 64, r);
    let nStage3 = dStage3.length;
    if (nStage3 > 63)
        return;
    let b0 = new Int32Array(16);
    b0[0] = nStage3 << 3;
    let b = new Uint8Array(64);
    for (let i = 0; i < nStage3; i++)
        b[i] = dStage3[i];
    b[nStage3] = 0x01;
    let m = get512(b), m0 = get512(b0);
    g(h, N, m);
    add512(N, m0);
    add512(sigma, m);
    g(h, buffer0, N);
...
const signHMAC = (key, data) => {
    ...
    if (k.byteLength === b)
        ...
        k0 = new Uint8Array(b);
        if (k.byteLength > b) {
            k0.set(new Uint8Array(stribog(k)));
        } else {
    ...
    s1.set(new Uint8Array(stribog(s0)), b);
    return stribog(s1);
};

export default (password, salt, globalSalt, conf) => {
    ...
    let slen = salt.byteLength + globalSalt.byteLength,
        data = new Uint8Array(slen + 4);

    arraycopy(salt, 0, data, 0, slen);
    ...
    for (let i = 1; i <= n; i++) {
        ...
        let U = new Uint8Array(signHMAC(password, data)), Z = U;

        for (let j = 1; j < iterations; j++) {
            U = new Uint8Array(signHMAC(password, U));
